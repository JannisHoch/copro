...
def nc_with_float_timestamp(migration_gdf, config, root_dir, var_name, sim_year):
    ...
    for i in range(len(migration_gdf_crs_corrected)):
        ...
        if not zonal_stats:
            print("WARNING: No valid statistics found for polygon:", polygon.GID_2)
            # Decide whether to skip the polygon or assign a default value to val. For example, assign np.nan or 0
            val = 0  # or np.nan
        else:
...
def nc_with_continous_datetime_timestamp(migration_gdf, config, root_dir, var_name, sim_year):
    ...
    for i in range(len(migration_gdf_crs_corrected)):
        ...
        zonal_stats = rstats.zonal_stats(polygon.geometry, nc_arr_vals, affine=affine, stats=stat_method, all_touched=False, nodata=np.nan)
