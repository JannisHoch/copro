...
def get_geodataframe(
    config, root_dir, longitude="longitude", latitude="latitude", crs="EPSG:4326"
):
    ...
    if config.getboolean("general", "verbose"):
        click.echo("DEBUG: translating to geopandas dataframe")
...
def parse_settings(settings_file: str) -> RawConfigParser:
...
def parse_projection_settings(config, root_dir):
    ...
    for (each_key, each_val) in config.items("PROJ_files"):
        ...
        if config.getboolean("general", "verbose"):
            click.echo("DEBUG: parsing settings from file {}".format(each_val))
...
def make_output_dir(config, root_dir, config_dict):
    ...
    assert len(all_out_dirs) == len(config_dict), AssertionError(
        "ERROR: number of output folders and config-objects do not match!"
    )
        ...
        # else, remove all files with a few exceptions
            ...
            for root, _, files in os.walk(d):
                if (config.getboolean("general", "verbose")) and (len(files) > 0):
                    click.echo(
                        "DEBUG: remove files in {}".format(os.path.abspath(root))
                    )
                for fo in files:
                    if fo in ["XY.npy", "X.npy"]:
                        if config.getboolean("general", "verbose"):
                            click.echo("DEBUG: sparing {}".format(fo))
                    else:
...
def print_model_info():
    ...
    click.echo("")
    click.echo(
        click.style("#### CoPro version {} ####".format(copro.__version__), fg="yellow")
    )
    click.echo(
        click.style(
    ...
    click.echo(
        click.style(
            "#### Copyright (2020-{}): {} ####".format(
                date.today().year, copro.__author__
            ),
            fg="yellow",
        )
    )
    click.echo(
        click.style("#### Contact via: {} ####".format(copro.__email__), fg="yellow")
    )
    click.echo(
        click.style(
...
def initiate_setup(settings_file, verbose=None):
    ...
    and, if specified, downloads conflict data.

    Args:
        settings_file (str): path to settings-file (cfg-file).
        verbose (bool, optional): whether model is verbose or not, e.g. click.echos DEBUG output or not. \
            If None, then the setting in cfg-file counts. \
                Otherwise verbose can be set directly to function which superseded the cfg-file. Defaults to None.

    Returns:
        ConfigParser-object: parsed model configuration.
        out_dir_list: list with paths to output folders; \
            first main output folder, then reference run folder, then (multiple) folders for projection runs.
    ...
    click.echo(
        "INFO: verbose mode on: {}".format(config.getboolean("general", "verbose"))
    )
    ...
    if config.getboolean("general", "verbose"):
        click.echo(
            "DEBUG: copying cfg-file {} to folder {}".format(
                os.path.abspath(settings_file), main_dict["_REF"][1]
            )
        )
    copyfile(
    ...
    if config["conflict"]["conflict_file"] == "download":
        raise NotImplementedError(
            "Downloading conflict data is not implemented anymore."
        )

    # if any other model than all_data is specified, set number of runs to 1
    if (config.getint("general", "model") == 2) or (
        config.getint("general", "model") == 3
    ):
        config.set("machine_learning", "n_runs", str(1))
        click.echo(
            "INFO: changed nr of runs to {}".format(
                config.getint("machine_learning", "n_runs")
            )
        )
...
def determine_projection_period(config_REF, config_PROJ):
    ...
    Args:
        config_REF (ConfigParser-object): object containing the parsed \
            configuration-settings of the model for the reference run.
        config_PROJ (ConfigParser-object): object containing the parsed \
            configuration-settings of the model for a projection run..
    ...
    print(
        "INFO: the projection period is {} to {}".format(
            projection_period[0], projection_period[-1]
        )
    )
